...
typedef struct {
  uint8_t  node_id;
  uint8_t  sensor_type;   // 1=SUHU,2=KELEMBAPAN,9=HELLO,10=TIME_SYNC
  float    value;
  uint32_t timestamp;
} NodePacket;
...
void onReceive(const esp_now_recv_info_t*, const uint8_t* data, int len){
  ...
  NodePacket p; memcpy(&p,data,sizeof(p));
  if(p.sensor_type==10){
    base_epoch=p.timestamp-millis()/1000;
    time_ready=true;
    Serial.printf("[NODE 2] TIME | SYNC=OK | BASE=%lu | CH=%d\n",base_epoch,GW_CHANNEL);
  }
}
...
void sendHello(){
  ...
  Serial.printf("[NODE 2] TIME | REQUEST | CH=%d\n",GW_CHANNEL);
}
...
void sendDHT(){
  ...
  if(!isnan(t)){
    ...
    Serial.printf("[NODE 2] SEND SUHU | %.2f | TS=%lu | OK\n",t,ts);
  }
  if(!isnan(h)){
    esp_now_send(GATEWAY_MAC,(uint8_t*)&ph,sizeof(ph));
    Serial.printf("[NODE 2] SEND KELEMBAPAN | %.2f | TS=%lu | OK\n",h,ts);
  }
}
...
void setup(){
  ...
  dht.begin();
  Serial.printf("[NODE 2] ESPNOW | INIT OK | CH=%d\n",GW_CHANNEL);
  sendHello();
}
...
void loop(){
  ...
  if(millis()-last_send>2000){ last_send=millis(); sendDHT(); }
}
